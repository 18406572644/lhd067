...
async function handleExport() {
  ...
  try {
    ...
    const sampleData = new Blob(['Sample export data'], { type: 'text/plain' })
    const filename = project.value ? `${project.value.name}.txt` : 'herbarium-export.txt'
    downloadBlob(sampleData, filename)
...
<template>
  <div class="h-[calc(100vh-57px)] flex gap-6 p-6">
    <div class="flex-1 flex flex-col items-center justify-center bg-herb-cream-dark/20 rounded-3xl">
    ...
    <div class="w-80 flex flex-col gap-4">
      <div class="bg-white/60 backdrop-blur rounded-2xl p-4 border border-herb-brown-light/30">
        ...
        <div class="space-y-3">
          <div class="flex items-center justify-between">
            ...
            <span class="text-sm text-herb-brown font-medium">{{ project?.name || '未命名方案' }}</span>
          </div>
          <div class="flex items-center justify-between">
      ...
      <button
        class="w-full inline-flex items-center justify-center gap-2 px-6 py-2.5 rounded-full font-serif text-sm font-semibold transition-all duration-300 ease-out shadow-sm hover:shadow-lg active:scale-[0.97] bg-transparent border-2 border-herb-purple text-herb-purple hover:bg-herb-purple/10 hover:scale-[1.03]"
        @click="handleBack"
      >
